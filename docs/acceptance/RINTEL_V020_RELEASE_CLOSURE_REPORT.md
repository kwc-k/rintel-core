...
Known limitation: multiple distinct C source subjects currently create successive revision-bound overlays. Earlier support cannot be treated as current; the receipt labels it `SUPERSEDED`, and LVS must remain `UNKNOWN` where support is insufficient. This release only claims the tested bounded direct-call path. See `issue_register.json`.
